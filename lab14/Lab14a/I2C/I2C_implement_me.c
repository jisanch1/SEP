...
void i2cSendStop()
{
	DDRC &= ~_BV(DDC4);	// SDA to input.

	TWCR = _BV(TWINT) | _BV(TWSTO) | _BV(TWEN);	// Transmit STOP condition.
}
...
void i2cSendByte(unsigned char data)
{
	...
	TWDR = data;	// write data on the i/o register.

	TWCR = _BV(TWINT) | _BV(TWEN);	// Send.
}
...
void i2cReceiveByte(bool sendAnACK)
{
	...
	if (sendAnACK)
	{
		TWCR |= _BV(TWINT) | _BV(TWEA);	// Send ACK.
	}
	else
	{
		TWCR |= _BV(TWINT);	// Send NACK.
	}
}
